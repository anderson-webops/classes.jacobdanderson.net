...
Every route follows the effective Fall 2025 framework: Unit 1 Using Objects and Methods, Unit 2 Selection and Iteration, Unit 3 Class Creation, and Unit 4 Data Collections. The current exam is fully digital, with 42 multiple-choice questions and four typed free-response questions.

The course can move slowly, normally, quickly, or as a challenge track; the track is chosen from evidence, not confidence alone.

The track can change during the course. A learner might use the Quick Track for APCS1-4, slow down for class creation, use the Challenge Track for data collections, and switch to the Exam Track near the AP test.
...
5. Re-check the track after every major concept group: objects and methods, selection and iteration, class creation, data collections, and exam practice.
...
**Default sequence:** Complete the setup and placement check, then follow APCS1-4, Check-In #2, APCS5-6, Check-In #3, APCS9-11, Check-In #4, and APCS12-17. Use the inheritance/polymorphism appendix only when enrichment time remains.
...
**Default sequence:** Diagnose APCS1-4 quickly, backfill only Java-specific gaps, then spend most time on APCS5-6, APCS9-12, and APCS13-16.
...
4. **AP tracing:** Method calls, loops, arrays, `ArrayList` mutation, rectangular 2D arrays, recursion, searching, and sorting can be traced by hand without relying on the compiler.
...
| Standard Track  | Medium | Solid programming background with a need for a predictable sequence    | Complete one strong project per module and replace duplicate drills with AP-style checks.             | Objects, selection, iteration, class creation, data collections, recursion, searching, sorting, and exam practice.  |
| Quick Track     | Fast   | Strong Python/C++ background, high independence, quick syntax transfer | Treat APCS1-4 as diagnostics, then focus on class creation, collections, algorithms, and FRQs.        | `.equals()` vs `==`, integer division, casting, AP tracing, constructors, text-file input, arrays, and `ArrayList`. |
...
| Standard  | Duplicate drills and extra near-identical projects.        | Class creation, arrays, `ArrayList`, rectangular 2D arrays, recursion, and sorting. | One representative project per module plus a short FRQ or trace.         | The learner can state the AP concept, one edge case, and one likely scoring detail.            |
| Quick     | Most APCS1-4 repetition after diagnostics.                 | APCS5-6, APCS9-12, APCS13-16.                                                       | One strong Java build per major concept area.                            | The learner passes the Java-specific diagnostic and can trace object/collection state by hand. |
...
| Optional inheritance appendix   | Use only after Unit 3 core evidence is stable.                    | Optional enrichment.                         | Skip unless Java breadth is a separate goal.                  | Build a small hierarchy and label it outside AP scope.                  | Skip for current exam preparation.                    |
| APCS9 Specs and files           | Translate requirements into tests and trace text-file input.      | Required Unit 4 opening.                     | Short specification and text-file checkpoint.                 | Add provenance checks and deterministic file fixtures.                  | Trace `File` and `Scanner` behavior in supplied code. |
| APCS10 Arrays                   | Full traversal and mutation practice.                             | Required core.                               | Required core.                                                | Add helper methods and edge-case arrays.                                | Trace 1D and rectangular 2D indexing.                 |
...
4. APCS9 establishes specification reading, responsible computing, and text-file input with `File` and `Scanner`.
5. APCS10-12 cover array, rectangular 2D-array, `ArrayList`, and wrapper-class reasoning; arrays still matter even when Python lists or C++ vectors are familiar.
6. APCS13-16 form the high-value advanced stretch: linear search, binary search, selection sort, insertion sort, merge sort, and recursion.
7. APCS17 works best after the algorithm and object-modeling pieces can be explained without compiler feedback.
8. The inheritance/polymorphism appendix is optional Java enrichment after Unit 3, not a current AP exam prerequisite.
...
5. The following session can move to APCS9 text-file/specification work or APCS10 arrays once the object model is stable.
...
5. Use documented text-file fixtures when practicing required Unit 4 file input; keep keyboard interfaces as optional enrichment.
...
1. **Generics, interfaces, records, and inheritance:** Useful for broader Java design after class creation and `ArrayList` work are reliable; designing inheritance relationships remains outside current AP scope.
2. **Unit tests:** A lightweight unit-test or manual test harness can replace repetitive worksheets for advanced learners.
3. **Broader file input/output:** Unit 4 includes text-file reading with `File` and `Scanner`; file writing, binary formats, and broader persistence designs remain extensions.
...
4. **Text-file input and responsible computing:** read documented text files with `File` and `Scanner`, close resources, respect data provenance and privacy, and verify permitted AI assistance independently.
5. **Arrays and `ArrayList`:** traversal, mutation, insertion, removal, wrapper types, rectangular two-dimensional arrays, and off-by-one control.
6. **Algorithms:** linear search, binary search, selection sort, insertion sort, merge sort, recursion, and intermediate-state reasoning.
7. **Digital AP reasoning:** type code without compiler feedback, explain object state, use the current Java Quick Reference, and write partial-credit-friendly responses for all four FRQ families.
...
| Optional Java hierarchy | `Book/PictureBook` or `Many Shapes`          | Label as enrichment and trace declared type versus actual method dispatch.  |
